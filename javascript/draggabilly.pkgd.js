...
(function(window, factory) {
  ...
  proto.measureContainment = function() {
    ...
    if (f_IsFullScreen()) { //如果是全屏状态下，重新定义
      ...
      zoom = Math.floor(dbcw / 12.8) / 100;
    }

    this.containSize = {
      width: (containerSize.width * zoom - borderSizeX) - position.x - elemSize.width,
      height: (containerSize.height * zoom - borderSizeY) - position.y - elemSize.height
    };
  ...
  proto.dragMove = function(event, pointer, moveVector) {
    ...
    if (f_IsFullScreen()) { //如果是全屏状态下，重新定义
      ...
      dragX = dragX / zoom;
      dragY = dragY / zoom;
    } else if ($("body").attr("zoom") != undefined) {
      zoom = $("body").attr("zoom");
      if (zoom !== 0) {
        dragX = dragX / zoom;
        dragY = dragY / zoom;
  ...
  proto.setLeftTop = function(flag) {
    if (flag) {
